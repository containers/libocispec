...
def parse_map_string_obj(obj, c_file, prefix, obj_typename):
    ...
    c_file.write(f'        ret->{child.fixname} = calloc (len + 1, sizeof (*ret->{child.fixname}));\n')
    c_file.write(f'        if (ret->{child.fixname} == NULL)\n')
    c_file.write('          return NULL;\n')
    ...
    c_file.write(f'            ret->{child.fixname}[i] = make_{childname} (val, ctx, err);\n')
    c_file.write(f'            if (ret->{child.fixname}[i] == NULL)\n')
    c_file.write("              return NULL;\n")
    c_file.write('          }\n')
    c_file.write('      }\n')
...
def parse_obj_type_array(obj, c_file, prefix, obj_typename):
    if obj.subtypobj or obj.subtyp == 'object':
        ...
        c_file.write('    do\n')
        c_file.write('      {\n')
        c_file.write(f'        yajl_val tmp = get_val (tree, "{obj.origname}", yajl_t_array);\n')
        ...
        c_file.write(f'            ret->{obj.fixname}_len = len;\n')
        c_file.write(f'            ret->{obj.fixname} = calloc (len + 1, sizeof (*ret->{obj.fixname}));\n')
        c_file.write(f'            if (ret->{obj.fixname} == NULL)\n')
        c_file.write('              return NULL;\n')
        if obj.doublearray:
            c_file.write(f'            ret->{obj.fixname}_item_lens = calloc ( len + 1, sizeof (size_t));\n')
            c_file.write(f'            if (ret->{obj.fixname}_item_lens == NULL)\n')
            c_file.write('                return NULL;\n')
        c_file.write('            for (i = 0; i < len; i++)\n')
        c_file.write('              {\n')
        ...
        if obj.doublearray:
            ...
            c_file.write(f'                ret->{obj.fixname}[i] = calloc ( YAJL_GET_ARRAY_NO_CHECK(val)->len + 1, sizeof (**ret->{obj.fixname}));\n')
            c_file.write(f'                if (ret->{obj.fixname}[i] == NULL)\n')
            c_file.write('                    return NULL;\n')
            c_file.write('                yajl_val *items = YAJL_GET_ARRAY_NO_CHECK(val)->values;\n')
            c_file.write('                for (j = 0; j < YAJL_GET_ARRAY_NO_CHECK(val)->len; j++)\n')
            c_file.write('                  {\n')
            c_file.write(f'                    ret->{obj.fixname}[i][j] = make_{typename} (items[j], ctx, err);\n')
            c_file.write(f'                    if (ret->{obj.fixname}[i][j] == NULL)\n')
            c_file.write("                        return NULL;\n")
            c_file.write(f'                    ret->{obj.fixname}_item_lens[i] += 1;\n')
            c_file.write('                  };\n')
        else:
            c_file.write(f'                ret->{obj.fixname}[i] = make_{typename} (val, ctx, err);\n')
            c_file.write(f'                if (ret->{obj.fixname}[i] == NULL)\n')
            c_file.write("                  return NULL;\n")
        c_file.write('              }\n')
        c_file.write('          }\n')
        ...
        c_file.write('    do\n')
        c_file.write('      {\n')
        c_file.write(f'        yajl_val tmp = get_val (tree, "{obj.origname}", yajl_t_string);\n')
        c_file.write('        if (tmp != NULL)\n')
        c_file.write('          {\n')
        if obj.doublearray:
            c_file.write('                yajl_val *items = YAJL_GET_ARRAY_NO_CHECK(tmp)->values;\n')
            c_file.write(f'                ret->{obj.fixname} = calloc ( YAJL_GET_ARRAY_NO_CHECK(tmp)->len + 1, sizeof (*ret->{obj.fixname}));\n')
            c_file.write(f'                if (ret->{obj.fixname}[i] == NULL)\n')
            c_file.write('                    return NULL;\n')
            c_file.write('                size_t j;\n')
            ...
            c_file.write(f'                    ret->{obj.fixname}[j] = (uint8_t *)strdup (str ? str : "");\n')
            c_file.write(f'                    if (ret->{obj.fixname}[j] == NULL)\n')
            c_file.write("                        return NULL;\n")
            c_file.write('                  };\n')
        else:
            c_file.write('            char *str = YAJL_GET_STRING (tmp);\n')
            c_file.write(f'            ret->{obj.fixname} = (uint8_t *)strdup (str ? str : "");\n')
            c_file.write(f'            if (ret->{obj.fixname} == NULL)\n')
            c_file.write('              return NULL;\n')
            c_file.write(f'            ret->{obj.fixname}_len = str != NULL ? strlen (str) : 0;\n')
        c_file.write('        }\n')
        c_file.write('      }\n')
        c_file.write('    while (0);\n')
    else:
        ...
        if obj.doublearray:
            ...
            c_file.write(f'                    ret->{obj.fixname}[i] = calloc ( YAJL_GET_ARRAY_NO_CHECK(values[i])->len + 1, sizeof (**ret->{obj.fixname}));\n')
            c_file.write(f'                    if (ret->{obj.fixname}[i] == NULL)\n')
            c_file.write('                        return NULL;\n')
            c_file.write('                    size_t j;\n')
...
def parse_obj_type(obj, c_file, prefix, obj_typename):
    ...
    if obj.typ == 'booleanPointer':
        ...
        c_file.write(f"    if (ret->{obj.fixname} == NULL && *err != 0)\n")
        c_file.write("      return NULL;\n")
    elif obj.typ == 'array':
        ...
        c_file.write('    do\n')
        c_file.write('      {\n')
        c_file.write(f'        yajl_val tmp = get_val (tree, "{obj.origname}", yajl_t_object);\n')
        c_file.write('        if (tmp != NULL)\n')
        c_file.write('          {\n')
        c_file.write(f'            ret->{obj.fixname} = make_{helpers.make_basic_map_name(obj.typ)} (tmp, ctx, err);\n')
        c_file.write(f'            if (ret->{obj.fixname} == NULL)\n')
        c_file.write('              {\n')
        c_file.write('                char *new_error = NULL;\n')
        c_file.write(f"                if (asprintf (&new_error, \"Value error for key '{obj.origname}': %s\", *err ? *err : \"null\") < 0)\n")
...
def parse_obj_arr_obj(obj, c_file, prefix, obj_typename):
    ...
    for i in required_to_check:
        c_file.write(f'    if (ret->{i.fixname} == NULL)\n')
        c_file.write('      {\n')
        c_file.write(f'        if (asprintf (err, "Required field \'%s\' not present",  "{i.origname}") < 0)\n')
...
def parse_json_to_c(obj, c_file, prefix):
    ...
    if obj.typ == 'array':
        ...
        if objs is None or obj.subtypname:
            return
    c_file.write(f"define_cleaner_function ({typename} *, free_{typename})\n")
    c_file.write(f"{typename} *\nmake_{typename} (yajl_val tree, const struct parser_context *ctx, parser_error *err)\n")
    c_file.write("{\n")
    c_file.write(f"    __auto_cleanup(free_{typename}) {typename} *ret = NULL;\n")
...
def get_map_string_obj(obj, c_file, prefix):
    ...
    c_file.write(f'    if (len || (ptr != NULL && ptr->keys != NULL && ptr->{child.fixname} != NULL))\n')
    c_file.write('      {\n')
    c_file.write('        for (i = 0; i < len; i++)\n')
    c_file.write('          {\n')
...
(const unsigned char *)str, strlen (str));\n')
    c_file.write("            if (stat != yajl_gen_status_ok)\n")
    c_file.write("                GEN_SET_ERROR_AND_RETURN (stat, err);\n")
    c_file.write(f'            stat = gen_{childname} (g, ptr->{child.fixname}[i], ctx, err);\n')
    c_file.write("            if (stat != yajl_gen_status_ok)\n")
    c_file.write("                GEN_SET_ERROR_AND_RETURN (stat, err);\n")
    c_file.write('          }\n')
    c_file.write('      }\n')
...
def get_obj_arr_obj_array(obj, c_file, prefix):
    if obj.subtypobj or obj.subtyp == 'object':
        ...
        if obj.subtypname:
            ...
            typename = helpers.get_name_substr(obj.name, prefix)
        c_file.write(f'    if ((ctx->options & OPT_GEN_KEY_VALUE) || (ptr != NULL && ptr->{obj.fixname} != NULL))\n')
        c_file.write('      {\n')
        c_file.write('        size_t len = 0, i;\n')
        c_file.write(f'        stat = yajl_gen_string ((yajl_gen) g, (const unsigned char *)("{obj.origname}"), {int(l)} /* strlen ("{obj.origname}") */);\n')
        c_file.write("        if (stat != yajl_gen_status_ok)\n")
        c_file.write("            GEN_SET_ERROR_AND_RETURN (stat, err);\n")
        c_file.write(f"        if (ptr != NULL && ptr->{obj.fixname} != NULL)\n")
        c_file.write(f"            len = ptr->{obj.fixname}_len;\n")
        c_file.write("        if (!len && !(ctx->options & OPT_GEN_SIMPLIFY))\n")
        c_file.write('            yajl_gen_config (g, yajl_gen_beautify, 0);\n')
        ...
        if obj.doublearray:
            ...
            c_file.write("                GEN_SET_ERROR_AND_RETURN (stat, err);\n")
            c_file.write("            size_t j;\n")
            c_file.write(f'            for (j = 0; j < ptr->{obj.fixname}_item_lens[i]; j++)\n')
            c_file.write('              {\n')
            c_file.write(f'                stat = gen_{typename} (g, ptr->{obj.fixname}[i][j], ctx, err);\n')
            c_file.write("                if (stat != yajl_gen_status_ok)\n")
            c_file.write("                    GEN_SET_ERROR_AND_RETURN (stat, err);\n")
            c_file.write('              }\n')
            c_file.write('            stat = yajl_gen_array_close ((yajl_gen) g);\n')
        else:
            c_file.write(f'            stat = gen_{typename} (g, ptr->{obj.fixname}[i], ctx, err);\n')
            c_file.write("            if (stat != yajl_gen_status_ok)\n")
            c_file.write("                GEN_SET_ERROR_AND_RETURN (stat, err);\n")
        c_file.write('          }\n')
        c_file.write('        stat = yajl_gen_array_close ((yajl_gen) g);\n')
        ...
        l = len(obj.origname)
        c_file.write(f'    if ((ctx->options & OPT_GEN_KEY_VALUE) || (ptr != NULL && ptr->{obj.fixname} != NULL && ptr->{obj.fixname}_len))\n')
        c_file.write('      {\n')
        c_file.write('        const char *str = "";\n')
        c_file.write('        size_t len = 0;\n')
        c_file.write(f'        stat = yajl_gen_string ((yajl_gen) g, (const unsigned char *)("{obj.origname}"), {l} /* strlen ("{obj.origname}") */);\n')
        c_file.write("        if (stat != yajl_gen_status_ok)\n")
        c_file.write("            GEN_SET_ERROR_AND_RETURN (stat, err);\n")
        if obj.doublearray:
            c_file.write('            stat = yajl_gen_array_open ((yajl_gen) g);\n')
            ...
            c_file.write("        {\n")
            c_file.write("            size_t i;\n")
            c_file.write(f"            for (i = 0; i < ptr->{obj.fixname}_len; i++)\n")
            c_file.write("              {\n")
            c_file.write(f"                if (ptr->{obj.fixname}[i] != NULL)\n")
            c_file.write(f"                    str = (const char *)ptr->{obj.fixname}[i];\n")
            c_file.write("                else ()\n")
            c_file.write("                    str = "";\n")
            c_file.write('                stat = yajl_gen_string ((yajl_gen) g, \
                    (const unsigned char *)str, strlen(str));\n')
            ...
            c_file.write(f"        if (ptr != NULL && ptr->{obj.fixname} != NULL)\n")
            c_file.write("          {\n")
            c_file.write(f"            str = (const char *)ptr->{obj.fixname};\n")
            c_file.write(f"            len = ptr->{obj.fixname}_len;\n")
            c_file.write("          }\n")
            c_file.write('        stat = yajl_gen_string ((yajl_gen) g, \
...
def get_obj_arr_obj(obj, c_file, prefix):
    ...
    if obj.typ == 'string':
        ...
        c_file.write("            GEN_SET_ERROR_AND_RETURN (stat, err);\n")
        c_file.write(f"        if (ptr != NULL && ptr->{obj.fixname} != NULL)\n")
        c_file.write(f"            str = ptr->{obj.fixname};\n")
        json_value_generator(c_file, 2, "str", 'g', 'ctx', obj.typ)
        c_file.write("      }\n")
    elif helpers.judge_data_type(obj.typ):
        c_file.write(f'    if ((ctx->options & OPT_GEN_KEY_VALUE) || (ptr != NULL && ptr->{obj.fixname}_present))\n')
        c_file.write('      {\n')
        if obj.typ == 'double':
            numtyp = 'double'
        ...
        l = len(obj.origname)
        c_file.write(f'        {numtyp} num = 0;\n')
        c_file.write(f'        stat = yajl_gen_string ((yajl_gen) g, (const unsigned char *)("{obj.origname}"), {l} /* strlen ("{obj.origname}") */);\n')
        c_file.write("        if (stat != yajl_gen_status_ok)\n")
        c_file.write("            GEN_SET_ERROR_AND_RETURN (stat, err);\n")
        c_file.write(f"        if (ptr != NULL && ptr->{obj.fixname})\n")
        c_file.write(f"            num = ({numtyp})ptr->{obj.fixname};\n")
        ...
        if numtyp == "":
            return
        l = len(obj.origname)
        c_file.write(f'        {helpers.get_map_c_types(numtyp)} num = 0;\n')
        c_file.write(f'        stat = yajl_gen_string ((yajl_gen) g, \
(const unsigned char *)("{obj.origname}"), {l} /* strlen ("{obj.origname}") */);\n')
        c_file.write("        if (stat != yajl_gen_status_ok)\n")
        c_file.write("            GEN_SET_ERROR_AND_RETURN (stat, err);\n")
        c_file.write(f"        if (ptr != NULL && ptr->{obj.fixname} != NULL)\n")
        c_file.write("          {\n")
        c_file.write(f"            num = ({helpers.get_map_c_types(numtyp)})*(ptr->{obj.fixname});\n")
        c_file.write("          }\n")
        ...
        c_file.write("            GEN_SET_ERROR_AND_RETURN (stat, err);\n")
        c_file.write(f"        if (ptr != NULL && ptr->{obj.fixname})\n")
        c_file.write(f"            b = ptr->{obj.fixname};\n")
        c_file.write("        \n")
        ...
        c_file.write("        if (stat != yajl_gen_status_ok)\n")
        c_file.write("            GEN_SET_ERROR_AND_RETURN (stat, err);\n")
        c_file.write(f'        stat = gen_{typename} (g, ptr != NULL ? ptr->{obj.fixname} : NULL, ctx, err);\n')
        c_file.write("        if (stat != yajl_gen_status_ok)\n")
        c_file.write("            GEN_SET_ERROR_AND_RETURN (stat, err);\n")
        c_file.write("      }\n")
    elif obj.typ == 'array':
        ...
        c_file.write(f'    if ((ctx->options & OPT_GEN_KEY_VALUE) || (ptr != NULL && ptr->{obj.fixname} != NULL))\n')
        c_file.write('      {\n')
        c_file.write(f'        stat = yajl_gen_string ((yajl_gen) g, (const unsigned char *)("{obj.fixname}"), {l} /* strlen ("{obj.fixname}") */);\n')
        c_file.write("        if (stat != yajl_gen_status_ok)\n")
        c_file.write("            GEN_SET_ERROR_AND_RETURN (stat, err);\n")
        c_file.write(f'        stat = gen_{helpers.make_basic_map_name(obj.typ)} (g, ptr ? ptr->{obj.fixname} : NULL, ctx, err);\n')
        c_file.write("        if (stat != yajl_gen_status_ok)\n")
        c_file.write("            GEN_SET_ERROR_AND_RETURN (stat, err);\n")
        c_file.write("      }\n")
...
def read_val_generator(c_file, level, src, dest, typ, keyname, obj_typename):
    ...
    if helpers.valid_basic_map_name(typ):
        c_file.write(f"{'    ' * level}yajl_val val = {src};\n")
        c_file.write(f"{'    ' * level}if (val != NULL)\n")
        c_file.write(f'{"    " * level}  {{\n')
        c_file.write(f'{"    " * (level + 1)}{dest} = make_{helpers.make_basic_map_name(typ)} (val, ctx, err);\n')
        c_file.write(f"{'    ' * (level + 1)}if ({dest} == NULL)\n")
        c_file.write(f'{"    " * (level + 1)}  {{\n')
        c_file.write(f"{'    ' * (level + 1)}    char *new_error = NULL;\n")
        c_file.write(f"{'    ' * (level + 1)}    if (asprintf (&new_error, \"Value error for key '{keyname}': %s\", *err ? *err : \"null\") < 0)\n")
        c_file.write(f'{"    " * (level + 1)}        new_error = strdup ("error allocating memory");\n')
        c_file.write(f"{'    ' * (level + 1)}    free (*err);\n")
        c_file.write(f"{'    ' * (level + 1)}    *err = new_error;\n")
        c_file.write(f"{'    ' * (level + 1)}    return NULL;\n")
        c_file.write(f'{"    " * (level + 1)}  }}\n')
        c_file.write(f'{"    " * (level)}}}\n')
    elif typ == 'string':
        c_file.write(f"{'    ' * level}yajl_val val = {src};\n")
        c_file.write(f"{'    ' * level}if (val != NULL)\n")
        c_file.write(f"{'    ' * (level)}  {{\n")
        c_file.write(f"{'    ' * (level + 1)}char *str = YAJL_GET_STRING (val);\n")
        c_file.write(f"{'    ' * (level + 1)}{dest} = strdup (str ? str : \"\");\n")
        c_file.write(f"{'    ' * (level + 1)}if ({dest} == NULL)\n")
        c_file.write(f"{'    ' * (level + 1)}  return NULL;\n")
        c_file.write(f'{"    " * level}  }}\n')
        ...
        if typ.startswith("uint") or \
            ...
            c_file.write(f"{'    ' * (level + 1)}int invalid;\n")
            c_file.write(f"{'    ' * (level + 1)}if (! YAJL_IS_NUMBER (val))\n")
            c_file.write(f'{"    " * (level + 1)}  {{\n')
            c_file.write(f"{'    ' * (level + 1)}    *err = strdup (\"invalid type\");\n")
            c_file.write(f"{'    ' * (level + 1)}    return NULL;\n")
            c_file.write(f'{"    " * (level + 1)}  }}\n')
            c_file.write(f'{"    " * (level + 1)}invalid = common_safe_{typ} (YAJL_GET_NUMBER (val), &{dest});\n')
        elif typ == "integer":
            c_file.write(f"{'    ' * (level + 1)}int invalid;\n")
            c_file.write(f"{'    ' * (level + 1)}if (! YAJL_IS_NUMBER (val))\n")
            c_file.write(f'{"    " * (level + 1)}  {{\n')
            ...
            c_file.write(f"{'    ' * (level + 1)}    return NULL;\n")
            c_file.write(f'{"    " * (level + 1)}  }}\n')
            c_file.write(f'{"    " * (level + 1)}invalid = common_safe_int (YAJL_GET_NUMBER (val), (int *)&{dest});\n')
            ...
            c_file.write(f"{'    ' * (level + 1)}    return NULL;\n")
            c_file.write(f'{"    " * (level + 1)}  }}\n')
            c_file.write(f'{"    " * (level + 1)}invalid = common_safe_uint (YAJL_GET_NUMBER (val), (unsigned int *)&{dest});\n')
        c_file.write(f"{'    ' * (level + 1)}if (invalid)\n")
        c_file.write(f'{"    " * (level + 1)}  {{\n')
        c_file.write(f'{"    " * (level + 1)}    if (asprintf (err, "Invalid value \'%s\' with type \'{typ}\' for key \'{keyname}\': %s", YAJL_GET_NUMBER (val), strerror (-invalid)) < 0)\n')
        c_file.write(f'{"    " * (level + 1)}        *err = strdup ("error allocating memory");\n')
        c_file.write(f"{'    ' * (level + 1)}    return NULL;\n")
        c_file.write(f'{"    " * (level + 1)}}}\n')
        if '[' not in dest:
            c_file.write(f"{'    ' * (level + 1)}{dest}_present = 1;\n")
        c_file.write(f'{"    " * (level)}}}\n')
        ...
        c_file.write(f"{'    ' * level}if (val != NULL)\n")
        c_file.write(f'{"    " * (level)}  {{\n')
        c_file.write(f'{"    " * (level + 1)}{dest} = calloc (1, sizeof ({helpers.get_map_c_types(num_type)}));\n')
        c_file.write(f"{'    ' * (level + 1)}if ({dest} == NULL)\n")
        c_file.write(f"{'    ' * (level + 1)}    return NULL;\n")
        c_file.write(f"{'    ' * (level + 1)}int invalid;\n")
        c_file.write(f"{'    ' * (level + 1)}if (! YAJL_IS_NUMBER (val))\n")
        ...
        c_file.write(f"{'    ' * (level + 1)}    return NULL;\n")
        c_file.write(f'{"    " * (level + 1)}}}\n')
        c_file.write(f'{"    " * (level + 1)}sinvalid = common_safe_{num_type} (YAJL_GET_NUMBER (val), {dest});\n')
        c_file.write(f"{'    ' * (level + 1)}if (invalid)\n")
        c_file.write(f'{"    " * (level + 1)}  {{\n')
        c_file.write(f'{"    " * (level + 1)}    if (asprintf (err, "Invalid value \'%s\' with type \'{typ}\' ' \
        ...
        c_file.write(f"{'    ' * level}if (val != NULL)\n")
        c_file.write(f'{"    " * (level)}  {{\n')
        c_file.write(f"{'    ' * (level + 1)}{dest} = YAJL_IS_TRUE(val);\n")
        if '[' not in dest:
            c_file.write(f"{'    ' * (level + 1)}{dest}_present = 1;\n")
            c_file.write(f'{"    " * (level)}  }}\n')
            c_file.write(f"{'    ' * level}else\n")
            c_file.write(f'{"    " * (level)}  {{\n')
            c_file.write(f"{'    ' * (level + 1)}val = {src.replace('yajl_t_true', 'yajl_t_false')};\n")
            c_file.write(f"{'    ' * (level + 1)}if (val != NULL)\n")
            c_file.write(f'{"    " * (level+1)}  {{\n')
            c_file.write(f"{'    ' * (level + 2)}{dest} = 0;\n")
            c_file.write(f"{'    ' * (level + 2)}{dest}_present = 1;\n")
            c_file.write(f'{"    " * (level+1)}  }}\n')
        c_file.write(f'{"    " * (level)}  }}\n')
        ...
        c_file.write(f"{'    ' * level}if (val != NULL)\n")
        c_file.write(f'{"    " * (level)}  {{\n')
        c_file.write(f"{'    ' * (level + 1)}{dest} = calloc (1, sizeof (bool));\n")
        c_file.write(f"{'    ' * (level + 1)}if ({dest} == NULL)\n")
        c_file.write(f"{'    ' * (level + 1)}    return NULL;\n")
        c_file.write(f"{'    ' * (level + 1)}*({dest}) = YAJL_IS_TRUE(val);\n")
        c_file.write(f'{"    " * (level)}  }}\n')
        c_file.write(f"{'    ' * level}else\n")
        c_file.write(f'{"    " * (level)} {{\n')
        c_file.write(f'{"    " * (level + 1)}val = get_val (tree, "{keyname}", yajl_t_false);\n')
        c_file.write(f"{'    ' * (level + 1)}if (val != NULL)\n")
        c_file.write(f'{"    " * (level + 1)}  {{\n')
        c_file.write(f"{'    ' * (level + 2)}{dest} = calloc (1, sizeof (bool));\n")
        c_file.write(f"{'    ' * (level + 2)}if ({dest} == NULL)\n")
        c_file.write(f"{'    ' * (level + 2)}  return NULL;\n")
        c_file.write(f"{'    ' * (level + 2)}*({dest}) = YAJL_IS_TRUE(val);\n")
        c_file.write(f'{"    " * (level + 1)}}}\n')
        c_file.write(f'{"    " * (level)}}}\n')
...
def json_value_generator(c_file, level, src, dst, ptx, typ):
    ...
    """
    if helpers.valid_basic_map_name(typ):
        c_file.write(f'{"    " * (level)}stat = gen_{helpers.make_basic_map_name(typ)} ({dst}, {src}, {ptx}, err);\n')
        c_file.write(f"{'    ' * level}if (stat != yajl_gen_status_ok)\n")
        c_file.write(f"{'    ' * (level + 1)}GEN_SET_ERROR_AND_RETURN (stat, err);\n")
    elif typ == 'string':
        c_file.write(f'{"    " * (level)}stat = yajl_gen_string ((yajl_gen){dst}, (const unsigned char *)({src}), strlen ({src}));\n')
        c_file.write(f"{'    ' * level}if (stat != yajl_gen_status_ok)\n")
        c_file.write(f"{'    ' * (level + 1)}GEN_SET_ERROR_AND_RETURN (stat, err);\n")
    elif helpers.judge_data_type(typ):
        if typ == 'double':
            c_file.write(f'{"    " * (level)}stat = yajl_gen_double ((yajl_gen){dst}, {src});\n')
        elif typ.startswith("uint") or typ == 'GID' or typ == 'UID':
            c_file.write(f"{'    ' * level}stat = map_uint ({dst}, {src});\n")
        else:
            c_file.write(f"{'    ' * level}stat = map_int ({dst}, {src});\n")
        c_file.write(f"{'    ' * level}if (stat != yajl_gen_status_ok)\n")
        c_file.write(f"{'    ' * (level + 1)}GEN_SET_ERROR_AND_RETURN (stat, err);\n")
    elif typ == 'boolean':
        c_file.write(f'{"    " * (level)}stat = yajl_gen_bool ((yajl_gen){dst}, (int)({src}));\n')
        c_file.write(f"{'    ' * level}if (stat != yajl_gen_status_ok)\n")
        c_file.write(f"{'    ' * (level + 1)}GEN_SET_ERROR_AND_RETURN (stat, err);\n")
...
def make_c_array_free (i, c_file, prefix):
    if helpers.valid_basic_map_name(i.subtyp):
        ...
        c_file.write(f"    if (ptr->{i.fixname} != NULL)\n")
        c_file.write("      {\n")
        c_file.write("        size_t i;\n")
        c_file.write(f"        for (i = 0; i < ptr->{i.fixname}_len; i++)\n")
        c_file.write("          {\n")
        c_file.write(f"            if (ptr->{i.fixname}[i] != NULL)\n")
        c_file.write("              {\n")
        c_file.write(f"                free_{free_func} (ptr->{i.fixname}[i]);\n")
        c_file.write(f"                ptr->{i.fixname}[i] = NULL;\n")
        c_file.write("              }\n")
        c_file.write("          }\n")
        c_file.write(f"        free (ptr->{i.fixname});\n")
        c_file.write(f"        ptr->{i.fixname} = NULL;\n")
        c_file.write("      }\n")
        ...
        if i.doublearray:
            ...
            c_file.write(f"            for (i = 0; i < ptr->{i.fixname}_len; i++)\n")
            c_file.write("              {\n")
            c_file.write(f"                free (ptr->{i.fixname}[i]);\n")
            c_file.write(f"                ptr->{i.fixname}[i] = NULL;\n")
            c_file.write("              }\n")
            c_file.write(f"            free (ptr->{i.fixname}_item_lens);\n")
            c_file.write(f"            ptr->{i.fixname}_item_lens = NULL;\n")
        c_file.write(f"        free (ptr->{i.fixname});\n")
        c_file.write(f"        ptr->{i.fixname} = NULL;\n")
        c_file.write("    }\n")
        ...
        if i.doublearray:
            c_file.write("          size_t j;\n")
            c_file.write(f"          for (j = 0; j < ptr->{i.fixname}_item_lens[i]; j++)\n")
            c_file.write("            {\n")
            c_file.write(f"              free_{free_func} (ptr->{i.fixname}[i][j]);\n")
            c_file.write(f"              ptr->{i.fixname}[i][j] = NULL;\n")
            c_file.write("          }\n")
            c_file.write(f"        free (ptr->{i.fixname}[i]);\n")
            c_file.write(f"        ptr->{i.fixname}[i] = NULL;\n")
        else:
    ...
    c_file.write(f"    free_{c_typ} (ptr->{i.fixname});\n")
    c_file.write(f"    ptr->{i.fixname} = NULL;\n")
    return False
...
def make_c_free (obj, c_file, prefix):
    ...
    for i in objs or []:
        if helpers.valid_basic_map_name(i.typ):
            ...
            c_file.write(f"    free_{free_func} (ptr->{i.fixname});\n")
            c_file.write(f"    ptr->{i.fixname} = NULL;\n")
        if i.typ == 'mapStringObject':
            ...
            if i.typ == 'string' or i.typ == 'booleanPointer' or \
                ...
                c_file.write(f"    if (ptr->{i.fixname} != NULL)\n")
                c_file.write("      {\n")
                c_file.write(f"        free_{typename} (ptr->{i.fixname});\n")
                c_file.write(f"        ptr->{i.fixname} = NULL;\n")
                c_file.write("      }\n")
    if obj.typ == 'object':
        if obj.children is not None:
...
def c_file_map_str(c_file, child, childname):
    ...
    History: 2019-10-31
    """
    c_file.write(f"    if (ptr->keys != NULL && ptr->{child.fixname} != NULL)\n")
    c_file.write("      {\n")
    c_file.write("        size_t i;\n")
    ...
    c_file.write(f"            free_{childname} (ptr->{child.fixname}[i]);\n")
    c_file.write(f"            ptr->{child.fixname}[i] = NULL;\n")
    c_file.write("          }\n")
    c_file.write("        free (ptr->keys);\n")
    c_file.write("        ptr->keys = NULL;\n")
    c_file.write(f"        free (ptr->{child.fixname});\n")
    c_file.write(f"        ptr->{child.fixname} = NULL;\n")
    c_file.write("      }\n")
...
def c_file_str(c_file, i):
    ...
    if i.doublearray:
        ...
        c_file.write(f"            for (j = 0; j < ptr->{i.fixname}_item_lens[i]; j++)\n")
        c_file.write("              {\n")
        c_file.write(f"                free (ptr->{i.fixname}[i][j]);\n")
        c_file.write(f"                ptr->{i.fixname}[i][j] = NULL;\n")
        c_file.write("            }\n")
    c_file.write(f"            if (ptr->{i.fixname}[i] != NULL)\n")
    c_file.write("              {\n")
...
def src_reflect(structs, schema_info, c_file, root_typ):
    ...
    History: 2019-06-17
    """
    c_file.write(f"/* Generated from {schema_info.name.basename}. Do not edit!  */\n\n")
    ...
    c_file.write(f'#include "ocispec/{schema_info.header.basename}"\n\n')
...
""");
    ...
    if obj.subtypobj or obj.subtyp == 'object':
        ...
        if obj.doublearray:
            ...
            c_file.write('          {\n')
            c_file.write(f'              ptr->items[i][j] = make_{subtypename} (tmps[j], ctx, err);\n')
            c_file.write('              if (ptr->items[i][j] == NULL)\n')
            c_file.write("                return NULL;\n")
            c_file.write('              ptr->subitem_lens[i] += 1;\n')
            c_file.write('          }\n')
        else:
            c_file.write(f'        ptr->items[i] = make_{subtypename} (work, ctx, err);\n')
            c_file.write('        if (ptr->items[i] == NULL)\n')
            c_file.write("          return NULL;\n")
    elif obj.subtyp == 'byte':
        if obj.doublearray:
...
""")
    ...
    if helpers.valid_basic_map_name(obj.subtyp):
        ...
        c_file.write("        if (ptr->items[i] != NULL)\n")
        c_file.write("          {\n")
        c_file.write(f"            free_{free_func} (ptr->items[i]);\n")
        ...
        if obj.doublearray:
            ...
            c_file.write("          for (j = 0; j < ptr->subitem_lens[i]; j++)\n")
            c_file.write("            {\n")
            c_file.write(f"              free_{free_func} (ptr->items[i][j]);\n")
            c_file.write("              ptr->items[i][j] = NULL;\n")
            c_file.write("            }\n")
            c_file.write("            free (ptr->items[i]);\n")
            c_file.write("            ptr->items[i] = NULL;\n")
...
""")
    ...
    if c_typ != "":
        if obj.subobj is not None:
            c_typ = c_typ + "_element"
        c_file.write(f"    free_{c_typ} (ptr->items);\n")
...
def get_c_epilog_for_array_make_gen(c_file, prefix, typ, obj):
    ...
    c_file.write(f"yajl_gen_status gen_{typename} (yajl_gen g, const {typename} *ptr, const struct parser_context *ctx," + """
                       parser_error *err)
{
...
""")
        ...
        if obj.doublearray:
            ...
            c_file.write('            for (j = 0; j < ptr->subitem_lens[i]; j++)\n')
            c_file.write('              {\n')
            c_file.write(f'                stat = gen_{subtypename} (g, ptr->items[i][j], ctx, err);\n')
            c_file.write("                if (stat != yajl_gen_status_ok)\n")
            c_file.write("                    GEN_SET_ERROR_AND_RETURN (stat, err);\n")
            c_file.write('              }\n')
            c_file.write('            stat = yajl_gen_array_close ((yajl_gen) g);\n')
        else:
            c_file.write(f'            stat = gen_{subtypename} (g, ptr->items[i], ctx, err);\n')
            c_file.write("            if (stat != yajl_gen_status_ok)\n")
            c_file.write("                GEN_SET_ERROR_AND_RETURN (stat, err);\n")
        c_file.write("""\n
            }
